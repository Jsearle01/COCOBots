...
**The gate path is `./run.sh` (or `run.bat`)** — a live, interactive MAME run with the FDC attached,
...
evidence, not the gate** — CLAUDE.md §4 classes a still as `static-png`, which verifies endpoints
...
**The live gate path — this is what 25.3 is taken against:**

```
./run.sh --auto            # or: run.bat --auto
```

Sets `-ext fdc` (mandatory, idioms §14a), mounts a **copy** of the disk (idioms §24), and applies
Monitor Type = RGB from `tools/mame-cfg/coco3.cfg` — a machine configuration, not a CLI flag, with
MAME defaulting to Composite (idioms §11l, §18). Verified applied, **with a negative control**:
...
The control matters — a config that silently fails to apply is exactly the §18 failure that
...
**Screenshots — supporting evidence only, surfaced for Jay and not interpreted (CLAUDE.md §3).**
...
7. **POST-REPORT CORRECTION — the gate deliverable was wrong, and Jay caught it.**
   ...
   **This was already written down and I under-weighted it.** CLAUDE.md §4 classes `static-png` as
   "**NOT a live gate** — verifies endpoints only; cannot show motion", and idioms §11 says visual
...
4. **The screenshots are in `build/`, which is gitignored — they are local-only.** Now a *minor*
   flag rather than a significant one: since the gate is a live run (§6 item 7), the stills are
   ...
   **They were also destroyed once and reported as present.** A manual
   ...
   Not a build-script bug — `build.sh`/`build.bat` only `mkdir -p build` and never delete it. The
...
1. *"i don't see build/a2c"* — the captures had been destroyed by my own clean-state test before I
   reported their paths (§7 flag 4). Regenerated; the run is deterministic and reproduced
...
2. *"verdict gate should always be a live run when possible. png stills are last resort for visual
   verification."* — a correction to what the gate deliverable is. Acted on in §6 item 7; produced
...
A fifth row was added after Jay's gate correction (§6 item 7), pool commit `8421c93`:

| slug | one line |
|---|---|
| `a-gate-artifact-is-not-a-gate-ship-the-live-path` | When the verdict belongs to a human, the deliverable is the means to exercise the system, not an artifact depicting it — an AC asking for a screenshot is evidence the run happened, not the gate |

**`initiator: orchestrator`** on that one — Jay raised it, I did not catch it, and per the pool's
§5 that distinction is load-bearing and must not be recorded as executor-originated.
...
| `6edeca9` | A2: add `run.sh` / `run.bat` — the LIVE gate path |
| branch | `wip`, pushed |
